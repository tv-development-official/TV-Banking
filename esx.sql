-- ═══════════════════════════════════════════════
--  tv-banking — ESX database schema
--  Import this file if your server runs ESX / ESX Legacy.
--  (account_ref stores each player's identifier)
-- ═══════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS `tv_banking_statements` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `account_type` ENUM('personal','society') NOT NULL DEFAULT 'personal',
  `account_ref` VARCHAR(64) NOT NULL COMMENT 'identifier for personal accounts, society name for society accounts',
  ...
  `actor` VARCHAR(64) DEFAULT '' COMMENT 'identifier of whoever performed the action',
...
CREATE TABLE IF NOT EXISTS `tv_banking_society` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(64) NOT NULL UNIQUE COMMENT 'matches a job name in the ESX jobs table (esx_society)',
...
CREATE TABLE IF NOT EXISTS `tv_banking_accounts` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `account_type` ENUM('personal','society') NOT NULL DEFAULT 'personal',
  `owner_ref` VARCHAR(64) NOT NULL COMMENT 'identifier for personal accounts, society name for society accounts',
...
CREATE TABLE IF NOT EXISTS `tv_banking_requests` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `requester_identifier` VARCHAR(64) NOT NULL COMMENT 'identifier of who is asking to be paid',
  `requester_name` VARCHAR(128) NOT NULL DEFAULT '',
  `requester_iban` VARCHAR(34) NOT NULL,
  `payer_identifier` VARCHAR(64) NOT NULL COMMENT 'identifier of who is being asked to pay',
  `amount` INT(11) NOT NULL DEFAULT 0,
  `reason` VARCHAR(255) DEFAULT '',
...
CREATE TABLE IF NOT EXISTS `tv_banking_cards` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `owner_ref` VARCHAR(64) NOT NULL COMMENT 'identifier',
...
-- ═══════════════════════════════════════════════
--  ITEM — the debit card players buy from the bank menu (Config.CardItem,
--  default 'bankcard'). ESX stores item definitions in the database rather
--  than a shared Lua file, so it needs registering here. INSERT IGNORE
--  makes this safe to re-run if the item already exists (e.g. added by
--  another resource, or this file imported twice) — change the `name`
--  below too if you change Config.CardItem.
-- ═══════════════════════════════════════════════
INSERT IGNORE INTO `items` (`name`, `label`, `weight`, `rare`, `can_remove`) VALUES ('bankcard', 'Debit Card', 1, 0, 1);
