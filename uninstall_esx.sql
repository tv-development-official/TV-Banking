-- ═══════════════════════════════════════════════
--  tv-banking — ESX uninstall script
...
-- Removes the debit card item definition this resource registered. Only
-- run this if no player inventories still hold one (check first — deleting
-- an item definition while it's still referenced in someone's inventory
-- can cause other scripts/ESX itself to error looking up its metadata).
DELETE FROM `items` WHERE `name` = 'bankcard';
